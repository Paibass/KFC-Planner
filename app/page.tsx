...
import { Calendar, Users, Clock, Trash2, History, TrendingUp, RefreshCw } from "lucide-react"
        ...
        <Card className="border-red-200 shadow-lg bg-gradient-to-r from-red-600 to-orange-600 text-white">
          <CardHeader className="text-center p-4 sm:p-6">
            ...
            <CardDescription className="text-lg sm:text-xl text-red-100 font-medium mt-2">
              🍗 Sistema Automático de Horarios con Fechas Reales
            </CardDescription>
          </CardHeader>
        </Card>
        ...
        <Card className="shadow-lg border-orange-200">
          <CardHeader className="bg-gradient-to-r from-orange-50 to-red-50 p-4 sm:p-6">
            ...
            <CardDescription className="text-base sm:text-lg mt-2">
              📄 Ingresa tu CUIL y carga el PDF más reciente
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4 sm:space-y-6 p-4 sm:pt-6">
            <div className="space-y-4 sm:grid sm:grid-cols-2 sm:gap-6 sm:space-y-0">
              <div className="space-y-2 sm:space-y-3">
                <Label htmlFor="cuil" className="text-base sm:text-lg font-semibold text-gray-700">
                  🆔 Tu CUIL (guardado automáticamente)
                </Label>
                <Input
                  id="cuil"
                  type="text"
            ...
            <Alert className="border-blue-200 bg-blue-50">
              ...
              <AlertDescription className="text-blue-800">
                <div className="font-semibold mb-2">🔄 Sistema Automático Activado:</div>
                <ul className="text-sm space-y-1">
                  <li>• Tu CUIL se guarda automáticamente</li>
                  <li>• Cada día se asocia a su fecha real (DD/MM/YYYY)</li>
                  <li>• Las quincenas se calculan automáticamente por fechas</li>
                  <li>• Los totales mensuales incluyen días de diferentes semanas</li>
                </ul>
              </AlertDescription>
            </Alert>

            {Object.keys(storedData.weeks).length > 0 && (
              <div className="space-y-3">
                ...
                <div className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                  <div className="text-sm text-gray-600 mb-2">
                    📊 Datos almacenados: {Object.keys(storedData.weeks).length} semanas,{" "}
                    {Object.keys(storedData.fortnights).length} quincenas, {Object.keys(storedData.months || {}).length}{" "}
                    meses
                  </div>
                </div>
              </div>
            ...
            {loading && (
              <Alert className="border-blue-200 bg-blue-50">
                ...
                <AlertDescription className="text-base sm:text-lg font-medium text-blue-800">
                  🔄 Procesando PDF con fechas reales y calculando totales automáticos...
                </AlertDescription>
              </Alert>
            )}
        ...
        {Object.keys(storedData.weeks).length > 0 && (
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="grid w-full grid-cols-4">
              <TabsTrigger value="current">📅 Semana</TabsTrigger>
              <TabsTrigger value="fortnight">📊 Quincena</TabsTrigger>
            ...
            <TabsContent value="current" className="space-y-4">
              {currentWeekData && currentUser && (
                <div className="space-y-4 sm:space-y-6">
                  ...
                  <Card className="shadow-lg border-blue-200">
                    <CardHeader className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 sm:p-6">
                      <CardTitle className="text-xl sm:text-2xl text-blue-700 flex items-center gap-2">
                        📅 Semana {currentWeekData.weekNumber} - {currentWeekData.year}
                      </CardTitle>
                      <CardDescription className="text-base sm:text-lg">📆 {currentWeekData.period}</CardDescription>
                    </CardHeader>
                    <CardContent className="p-4 sm:pt-6">
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        ...
                        <div className="p-4 bg-blue-50 rounded-lg border border-blue-200 text-center">
                          ...
                          <div className="text-sm font-semibold text-blue-600">⏰ Horas Totales</div>
                        </div>
                  ...
                  {/* Horarios del usuario con fechas reales */}
                  ...
                  <Card className="shadow-lg border-blue-200">
                    <CardHeader className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 sm:p-6">
                      ...
                      <CardDescription className="text-base sm:text-lg">
                        Compañeros que trabajan contigo
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="p-4 sm:pt-6">
                      <div className="space-y-3 sm:space-y-4">
                        {DAYS.map((day) => {
                          ...
                          const userSchedule =
                            currentUser.weeklySchedule[day as keyof typeof currentUser.weeklySchedule]
                ...
                <Card className="shadow-lg border-gray-200">
                  <CardContent className="p-8 text-center">
                    <div className="text-gray-500 text-lg">
                      📄 No hay datos de semana actual. Carga un PDF para comenzar.
                    </div>
                  </CardContent>
                </Card>
              )}
            ...
            {/* Vista de Quincena */}
            <TabsContent value="fortnight" className="space-y-4">
              {(() => {
                ...
                if (currentFortnight) {
                  return (
                    <Card className="shadow-lg border-purple-200">
                      <CardHeader className="bg-gradient-to-r from-purple-50 to-pink-50 p-4 sm:p-6">
                        <CardTitle className="text-xl sm:text-2xl text-purple-700 flex items-center gap-2">
                          <TrendingUp className="h-5 w-5 sm:h-6 sm:w-6" />📊 {currentFortnight.period}
                          <Badge className="bg-green-100 text-green-700 border-green-300">✅ Completa</Badge>
                        </CardTitle>
                        <CardDescription className="text-base sm:text-lg">
                          📅 Calculada automáticamente por fechas reales
                        </CardDescription>
                      </CardHeader>
                      <CardContent className="p-4 sm:pt-6">
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-6">
                          <div className="p-4 sm:p-6 bg-gradient-to-br from-green-50 to-green-100 rounded-xl border-2 border-green-200 text-center">
                            <div className="text-3xl sm:text-4xl font-bold text-green-700 mb-1 sm:mb-2">
                              {currentFortnight.totalDaysWorked}
                            </div>
                            <div className="text-sm sm:text-base font-semibold text-green-600">💼 Días Totales</div>
                          </div>
                          <div className="p-4 sm:p-6 bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl border-2 border-blue-200 text-center">
                            <div className="text-3xl sm:text-4xl font-bold text-blue-700 mb-1 sm:mb-2">
                              {currentFortnight.totalHours}h
                            </div>
                            <div className="text-sm sm:text-base font-semibold text-blue-600">⏰ Horas Quincena</div>
                          </div>
                          <div className="p-4 sm:p-6 bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl border-2 border-purple-200 text-center">
                            <div className="text-3xl sm:text-4xl font-bold text-purple-700 mb-1 sm:mb-2">
                              {currentFortnight.totalNightHours}h
                            </div>
                            <div className="text-sm sm:text-base font-semibold text-purple-600">
                              🌙 Nocturnas Quincena
                            </div>
                            <div className="text-xs text-purple-500 mt-1">(22:00 - 06:00)</div>
                          </div>
                        </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="grid gap-2">
                            {currentFortnight.dailyBreakdown.map((day) => (
                              <div key={day.date} className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                  <div className="font-medium text-gray-700">
                                    📅 {day.date} ({DAY_NAMES[day.dayName as keyof typeof DAY_NAMES]})
                                  </div>
                                  <div className="flex gap-4 text-sm">
                                    <span className="text-blue-600 font-medium">⏰ {day.schedule}</span>
                                    <span className="text-green-600 font-medium">{day.hours}h</span>
                                    {day.nightHours > 0 && (
                                      <span className="text-purple-600 font-medium">🌙 {day.nightHours}h</span>
                                    )}
                                  </div>
                                </div>
                              </div>
                  ...
                  return (
                    <Card className="shadow-lg border-gray-200">
                      <CardContent className="p-8 text-center">
                        <div className="text-gray-500 text-lg">
                          📊 No hay quincena completa aún. Carga más semanas para completar una quincena.
                        </div>
                      </CardContent>
                    </Card>
                  )
                ...
                if (currentMonth) {
                    ...
                    <Card className="shadow-lg border-indigo-200">
                      <CardHeader className="bg-gradient-to-r from-indigo-50 to-blue-50 p-4 sm:p-6">
                        <CardTitle className="text-xl sm:text-2xl text-indigo-700 flex items-center gap-2">
                          <Calendar className="h-5 w-5 sm:h-6 sm:w-6" />📆 {currentMonth.period}
                        </CardTitle>
                        <CardDescription className="text-base sm:text-lg">
                          📅 Resumen mensual con fechas exactas
                        </CardDescription>
                      </CardHeader>
                      <CardContent className="p-4 sm:pt-6">
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-6">
                          <div className="p-4 sm:p-6 bg-gradient-to-br from-green-50 to-green-100 rounded-xl border-2 border-green-200 text-center">
                            <div className="text-3xl sm:text-4xl font-bold text-green-700 mb-1 sm:mb-2">
                              {currentMonth.totalDaysWorked}
                            </div>
                            <div className="text-sm sm:text-base font-semibold text-green-600">💼 Días del Mes</div>
                          </div>
                          <div className="p-4 sm:p-6 bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl border-2 border-blue-200 text-center">
                            <div className="text-3xl sm:text-4xl font-bold text-blue-700 mb-1 sm:mb-2">
                              {currentMonth.totalHours}h
                            </div>
                            <div className="text-sm sm:text-base font-semibold text-blue-600">⏰ Horas Mensuales</div>
                          </div>
                          <div className="p-4 sm:p-6 bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl border-2 border-purple-200 text-center">
                            <div className="text-3xl sm:text-4xl font-bold text-purple-700 mb-1 sm:mb-2">
                              {currentMonth.totalNightHours}h
                            </div>
                            <div className="text-sm sm:text-base font-semibold text-purple-600">🌙 Nocturnas Mes</div>
                            <div className="text-xs text-purple-500 mt-1">(22:00 - 06:00)</div>
                          </div>
                        </div>

                        {/* Desglose mensual por fecha */}
                        <div className="space-y-3">
                          <h3 className="text-lg font-semibold text-gray-700 flex items-center gap-2">
                            📋 Todos los Días del Mes
                          </h3>
                          <div className="grid gap-2 max-h-96 overflow-y-auto">
                            {currentMonth.dailyBreakdown.map((day) => (
                              <div key={day.date} className="p-3 bg-indigo-50 rounded-lg border border-indigo-200">
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                  <div className="font-medium text-gray-700">
                                    📅 {day.date} ({DAY_NAMES[day.dayName as keyof typeof DAY_NAMES]})
                  ...
                  return (
                    <Card className="shadow-lg border-gray-200">
                      <CardContent className="p-8 text-center">
                        <div className="text-gray-500 text-lg">
                          📆 No hay datos mensuales aún. Carga semanas para generar resumen mensual.
                        </div>
                      </CardContent>
                    </Card>
                  )
            ...
            <TabsContent value="history" className="space-y-4">
              <div className="grid gap-4">
                ...
                <Card className="shadow-lg border-indigo-200">
                  <CardHeader className="bg-gradient-to-r from-indigo-50 to-blue-50 p-4 sm:p-6">
                    <CardTitle className="text-xl sm:text-2xl text-indigo-700 flex items-center gap-2">
                      <History className="h-5 w-5 sm:h-6 sm:w-6" />📚 Historial de Semanas
                    </CardTitle>
                    <CardDescription className="text-base sm:text-lg">
                      Todas las semanas cargadas ({Object.keys(storedData.weeks).length} semanas)
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="p-4 sm:pt-6">
                    <div className="space-y-3">
                      {Object.values(storedData.weeks)
                        .sort((a, b) => b.startDate.getTime() - a.startDate.getTime())
                        .map((week) => (
                          <div key={week.id} className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                              <div>
                                <div className="font-medium text-gray-700">
                                  📅 Semana {week.weekNumber} - {week.year}
                                </div>
                                <div className="text-sm text-gray-500">{week.period}</div>
                              </div>
                              <div className="flex gap-4 text-sm">
                                <span className="text-green-600 font-medium">
                                  💼 {week.userStats?.daysWorked || 0} días
                                </span>
                                <span className="text-blue-600 font-medium">⏰ {week.userStats?.totalHours || 0}h</span>
                                <span className="text-purple-600 font-medium">
                                  🌙 {week.userStats?.nightHours || 0}h
                                </span>
                              </div>
                            </div>
                          </div>
                ...
                {Object.keys(storedData.fortnights).length > 0 && (
                  <Card className="shadow-lg border-purple-200">
                    <CardHeader className="bg-gradient-to-r from-purple-50 to-pink-50 p-4 sm:p-6">
                      <CardTitle className="text-xl sm:text-2xl text-purple-700 flex items-center gap-2">
                        <TrendingUp className="h-5 w-5 sm:h-6 sm:w-6" />📊 Historial de Quincenas
                      </CardTitle>
                      <CardDescription className="text-base sm:text-lg">
                        Quincenas completadas ({Object.keys(storedData.fortnights).length} quincenas)
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="p-4 sm:pt-6">
                      <div className="space-y-3">
                        {Object.values(storedData.fortnights)
                          .sort((a, b) => new Date(b.completedAt).getTime() - new Date(a.completedAt).getTime())
                          .map((fortnight) => (
                            <div key={fortnight.id} className="p-4 bg-purple-50 rounded-lg border border-purple-200">
                              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                <div>
                                  <div className="font-medium text-purple-700">{fortnight.period}</div>
                                  <div className="text-sm text-purple-500">
                                    {fortnight.dailyBreakdown.length} días trabajados
                                  </div>
                                </div>
                                <div className="flex gap-4 text-sm">
                                  <span className="text-green-600 font-medium">
                                    💼 {fortnight.totalDaysWorked} días
                                  </span>
                                  <span className="text-blue-600 font-medium">⏰ {fortnight.totalHours}h</span>
                                  <span className="text-purple-600 font-medium">🌙 {fortnight.totalNightHours}h</span>
                                </div>
                              </div>
                            </div>
                          ))}
                ...
                {/* Historial de Meses */}
                {Object.keys(storedData.months || {}).length > 0 && (
                  <Card className="shadow-lg border-indigo-200">
                    <CardHeader className="bg-gradient-to-r from-indigo-50 to-blue-50 p-4 sm:p-6">
                      <CardTitle className="text-xl sm:text-2xl text-indigo-700 flex items-center gap-2">
                        <Calendar className="h-5 w-5 sm:h-6 sm:w-6" />📆 Historial de Meses
                      </CardTitle>
                      <CardDescription className="text-base sm:text-lg">
                        Resúmenes mensuales ({Object.keys(storedData.months || {}).length} meses)
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="p-4 sm:pt-6">
                      <div className="space-y-3">
                        {Object.values(storedData.months || {})
                          .sort((a, b) => b.year * 12 + b.month - (a.year * 12 + a.month))
                          .map((month) => (
                            <div key={month.id} className="p-4 bg-indigo-50 rounded-lg border border-indigo-200">
                              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                <div>
                                  <div className="font-medium text-indigo-700">{month.period}</div>
                                  <div className="text-sm text-indigo-500">
                                    {month.fortnights.length} quincenas, {month.dailyBreakdown.length} días trabajados
                                  </div>
                                </div>
                                <div className="flex gap-4 text-sm">
                                  <span className="text-green-600 font-medium">💼 {month.totalDaysWorked} días</span>
                                  <span className="text-blue-600 font-medium">⏰ {month.totalHours}h</span>
                                  <span className="text-purple-600 font-medium">🌙 {month.totalNightHours}h</span>
                                </div>
                              </div>
                            </div>
                          ))}
